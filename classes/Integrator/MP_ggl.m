classdef MP_ggl < Integrator
    ...
    methods
        ...
        function [resi, tang] = compute_resi_tang(self, zn1, zn, this_problem)
            ...
            %% Residual vector
            resi = [qn1 - qn - h * IM * p_n05 + h * G_n05' * gamma_n05; pn1 - pn + h * DV_n05 + h * G_n05' * lambda_n05; g_n1; G_n1 * IM * pn1];

            %% Tangent matrix
            tang = [eye(n) + h * (0.5) * t_n1mal, -h * (0.5) * IM, zeros(n, m), (0.5) * h * G_n05'; h * 0.5 * D2V_n05 + h * 0.5 * t_nal, eye(n), 0.5 * h * G_n05', zeros(n, m); G_n1, zeros(n, m)', zeros(m), zeros(m); T_n1, G_n1 * IM, zeros(m), zeros(m)];
