classdef GGL_VI_mod < Integrator
    ...
    methods
        ...
        function self = GGL_VI_mod(this_simulation, this_system)
        ...
        function [resi, tang] = compute_resi_tang(self, zn1, zn, this_system)
            ...
            %% Residual vector
            resi = [qn1 - qn - h * vn - h * IM * G_bar' * gamman1; pn1 - pn + h * DV_n + h * G_n' * lambdan + h * t_n_bar * IM * pn1; M * vn - pn1 - h * t_n_bar * IM * pn1; g_n1; G_bar * IM * pn1];

            %% Tangent matrix
            tang = [[eye(n), zeros(n), -h * eye(n) - h^2 * IM * t_n_bar, zeros(n, m), -h * IM * G_bar']; [zeros(n), eye(n) + h * t_n_bar * IM, zeros(n), h * G_n', h * T_bar']; [zeros(n), -eye(n) - h * t_n_bar * IM, M, zeros(n, m), -h * T_bar']; [G_n1, zeros(n, m)', zeros(n, m)', zeros(m), zeros(m)]; [zeros(n, m)', G_bar * IM, T_bar * h, zeros(m), zeros(m)]];
