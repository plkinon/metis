classdef Ggl_variational_2 < Integrator
...
% Date  : 09.12.2020
    ...
    methods
        ...
        function self = Ggl_variational_2(this_simulation,this_problem)
        ...
        function [resi,tang] = compute_resi_tang(self,zn1,zn,this_problem)
            ...
            %% Residual vector 
            resi = [qn1 - qn - h*IM*pn1 - h*IM*G_n'*gamman                  ;
                    pn1 - pn + h*DV_n + h*G_n'*lambdan + h*t_n*IM*pn1          ;
                    g_n1                                                      ;
                    G_n*IM*pn1                                              ];

            %% Tangent matrix
            %tang = [eye(n)                      -h*IM         zeros(n,m)   -h*IM*G_n' ;
            %        zeros(n)                    eye(n)+h*t_n*IM       h*G_n'        zeros(n,m)  ; 
            %        G_n1                        zeros(n,m)'  zeros(m)      zeros(m)    ;
            %        zeros(m,n)                      G_n*IM      zeros(m)      zeros(m)    ];
            tang=[];
        end
