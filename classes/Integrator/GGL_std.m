classdef GGL_std < Integrator
    ...
    methods
        ...
        function [resi, tang] = compute_resi_tang(self, zn1, zn, this_system)
            ...
            %% Residual vector
            resi = [qn1 - qn - h * IM * pn1 + h * G_n' * gamman; pn1 - pn + h * DV_n + h * G_n' * lambdan; g_n1; G_n1 * IM * pn1];

            %% Tangent matrix
            tang = [eye(n) + h * t_n, -h * IM, zeros(n, m), h * G_n'; zeros(n), eye(n), h * G_n', zeros(n, m); G_n1, zeros(n, m)', zeros(m), zeros(m); T_n1, G_n1 * IM, zeros(m), zeros(m)];
