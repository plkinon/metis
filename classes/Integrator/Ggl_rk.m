...
%             end
            %p0 = (M+h*(1-The)*t_0)*v0 + h*(1-The)*(DV_0 + G_0'*lambda0); 
            p0 = M*v0;
        ...
        function [resi,tang] = compute_resi_tang(self,zn1,zn,this_problem)
            ...
            %% Residual vector 
            resi = [qn1 - qn - h*vn - h*IM*G_nt'*gamman                  ;
                    pn1 - pn + h*DV_nt + h*((1-theta)*G_n'+theta*G_n1')*lambdan + h*t_n*vn;
                    (M + h * (1-The) * t_n) * vn - pn + h * (1-The) * (DV_nt + G_n' * lambdan) ;
                    g_n1                                                      ;
                    G_nt*vn                                              ];
